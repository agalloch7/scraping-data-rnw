...
class Review(object):
	...
	def __init__(self, review_dict, business=None):
		...
		self.user_name = review_dict['comment_author'] #string
		self.stars = int(review_dict['comment_likes']) #int
		self.text = review_dict['comment_message']	#string	
		...
		self.sentences = self.sentence_tokenize(self.text.decode('utf-8'))
